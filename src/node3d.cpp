...
bool Node3D::isOnGrid(const int width, const int height) const {
  return x_ >= 0 && x_ < width && y_ >= 0 && y_ < height && (int)(phi_ / Constants::deltaHeadingRad) >= 0 && (int)(phi_ / Constants::deltaHeadingRad) < Constants::headings;
}
...
bool Node3D::isInRange(const Node3D& goal) const {
  ...
  float dx = std::abs(x_ - goal.x_) / random;
  float dy = std::abs(y_ - goal.y_) / random;
  return (dx * dx) + (dy * dy) < Constants::dubinsShotDistance;
}

/**
 * @brief CREATE SUCCESSOR
 * @param i 
 * @return Node3D* 
 */
Node3D* Node3D::createSuccessor(const int i) {
  ...
  if (i < 3) {
    xSucc = x_ + dx[i] * cos(phi_) - dy[i] * sin(phi_);
    ySucc = y_ + dx[i] * sin(phi_) + dy[i] * cos(phi_);
    tSucc = Helper::normalizeHeadingRad(phi_ + dt[i]);
  }
  // backwards
  else {
    xSucc = x_ - dx[i - 3] * cos(phi_) - dy[i - 3] * sin(phi_);
    ySucc = y_ - dx[i - 3] * sin(phi_) + dy[i - 3] * cos(phi_);
    tSucc = Helper::normalizeHeadingRad(phi_ - dt[i - 3]);
  }
...
void Node3D::updateG() {
  ...
  if (prim_ < 3) {
    ...
    if (pred_->prim_ != prim_) {
      ...
      if (pred_->prim_ > 2) {
        g_ += dx[0] * Constants::penaltyTurning * Constants::penaltyCOD;
      } else {
        g_ += dx[0] * Constants::penaltyTurning;
      }
    } else {
      g_ += dx[0];
    }
  ...
  // reverse driving
    ...
    if (pred_->prim_ != prim_) {
      // penalize change of direction
      if (pred_->prim_ < 3) {
        g_ += dx[0] * Constants::penaltyTurning * Constants::penaltyReversing * Constants::penaltyCOD;
      } else {
        g_ += dx[0] * Constants::penaltyTurning * Constants::penaltyReversing;
      }
    } else {
      g_ += dx[0] * Constants::penaltyReversing;
    }
  }
}
...
bool Node3D::operator == (const Node3D& rhs) const {
  return (int)x_ == (int)rhs.x_ &&
         (int)y_ == (int)rhs.y_ &&
         (std::abs(phi_ - rhs.phi_) <= Constants::deltaHeadingRad ||
          std::abs(phi_ - rhs.phi_) >= Constants::deltaHeadingNegRad);
}
